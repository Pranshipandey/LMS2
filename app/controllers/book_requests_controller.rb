class BookRequestsController < ApplicationController
  ...
  def returns
    ...
    @book_requests = BookRequest.find(params[:id])
    @book = Book.find(@book_requests[:books_id])
    @student = Student.find(@book_requests[:students_id])
    @libr = Library.find(@book[:libraries_id])
    @get_rate = @libr[:fines]
    if @book_requests.nil?

    else
      @late_by = (Date.today.mjd-(@book_requests[:date].mjd+@libr[:max_days]))
      if(@late_by>0)
  ...
  def hold_requests
    @student = Student.find_by_email(current_user.email)
    @book_requests = BookRequest.find_by_sql(["select * from book_request where students_id = ? and hold = ?",@student.id,true])
    @book_requests_1 = BookRequest.find_by_sql(["select * from book_request where students_id = ? and is_approved = ? and hold = ?",@student.id,false,false])

  end

  def delete_hold_request
    @book_requests = BookRequest.find(params[:id])
    if(@book_requests.hold==false)
      @book = Book.find(@book_requests[:books_id])
